...
; 20/11/2023:	Added mos_api_runcpm_read, mos_api_runcpm_write, mos_api_runcpm_open, mos_api_runcpm_close, mos_api_runcpm_init, mos_api_runcpm_deinit
			...
			XREF	_mos_I2C_WRITE
			XREF	_mos_I2C_READ
			XREF	_mos_runcpm_read
			XREF	_mos_runcpm_write
			XREF	_mos_runcpm_open
			XREF	_mos_runcpm_close
			XREF	_mos_runcpm_init
			XREF	_mos_runcpm_deinit
...
mos_api_block1_start:	DW	mos_api_getkey		; 0x00
			...
			DW	mos_api_runcpm_read	; 0x23
			DW	mos_api_runcpm_write	; 0x24
			DW	mos_api_runcpm_open	; 0x25
			DW	mos_api_runcpm_close	; 0x26
			DW	mos_api_runcpm_init	; 0x27
			DW	mos_api_runcpm_deinit	; 0x28
...
ffs_api_setcp:		
			JP mos_api_not_implemented
;
; Read data from RunCPM
; HLU: Pointer to buffer to store the data
; BCU: Number of bytes to read
; Returns:
;   A: Number of bytes read
;
mos_api_runcpm_read:	LD	A, MB		; Check if MBASE is 0
			OR	A, A
			CALL	NZ, SET_AHL24
			PUSH	BC		; UINT24 btr
			PUSH	HL		; UINT24 buffer
			CALL	_mos_runcpm_read
			LD	A, L		; Number of bytes read
			POP	HL
			POP	BC
			RET
;
; Write data to RunCPM
; HLU: Pointer to buffer containing the data
; BCU: Number of bytes to write
; Returns:
;   A: Number of bytes written
;
mos_api_runcpm_write:	LD	A, MB		; Check if MBASE is 0
			OR	A, A
			CALL	NZ, SET_AHL24
			PUSH	BC		; UINT24 btw
			PUSH	HL		; UINT24 buffer
			CALL	_mos_runcpm_write
			LD	A, L		; Number of bytes written
			POP	HL
			POP	BC
			RET
;
; Open a file for RunCPM
; HLU: Pointer to filename (zero terminated)
;   C: Mode
; Returns:
;   A: Filehandle, or 0 if couldn't open
;
mos_api_runcpm_open:	LD	A, MB		; Check if MBASE is 0
			OR	A, A
			CALL	NZ, SET_AHL24
			PUSH	BC		; BYTE mode
			PUSH	HL		; const char * filename
			CALL	_mos_runcpm_open
			LD	A, L		; Filehandle
			POP	HL
			POP	BC
			RET
;
; Close a file for RunCPM
;   C: Filehandle
; Returns:
;   A: 0 if successful, otherwise error code
;
mos_api_runcpm_close:	PUSH	BC		; BYTE fh
			CALL	_mos_runcpm_close
			LD	A, L		; Error code
			POP	BC
			RET
;
...
;   A: 0 if successful, otherwise error code
;
mos_api_runcpm_init:	CALL	_mos_runcpm_init
			LD	A, L		; Error code
			RET
;
; Deinitialize communication with RunCPM
; Returns:
;   A: 0 if successful, otherwise error code
;
mos_api_runcpm_deinit:	CALL	_mos_runcpm_deinit
			LD	A, L		; Error code
			RET